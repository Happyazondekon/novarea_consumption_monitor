...
export default function UsersPage() {
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in py-4 lg:py-6 px-4 lg:px-6 text-left selection:bg-blue-500/30">
      <div id="team-header" className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-6 px-2">
        <div>
        ...
        {!showForm && (
            <button id="add-user-btn" onClick={() => handleOpenForm()} className="btn-primary flex items-center gap-2 px-8 py-3 rounded-xl shadow-lg shadow-blue-500/10 text-[10px] font-black uppercase">
      ...
      {showForm ? (
        <div id="user-form-container" className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[3rem] p-6 lg:p-10 space-y-8 shadow-xl animate-in slide-in-from-bottom-4 duration-500 mx-2">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    <div className="space-y-1.5"><label className="text-[9px] font-black text-zinc-400 uppercase ml-1">Operational Role</label><select value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl py-4 px-6 text-sm font-bold appearance-none outline-none text-zinc-900 dark:text-white"><option value="ELECTRICIEN">FIELD TECHNICIAN</option><option value="ADMINISTRATEUR">SYSTEM ADMINISTRATOR</option></select></div>
                    <div className="space-y-1.5">
                        ...
                        <input type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl py-4 px-6 text-sm font-bold text-zinc-900 dark:text-white" placeholder="••••••••" />
                    </div>
                ...
                <div className="md:col-span-2 pt-8 border-t border-zinc-200 dark:border-zinc-800 flex justify-end gap-4">
                    <button type="submit" disabled={loading} className="btn-primary px-12 py-4 rounded-2xl text-xs font-black shadow-xl shadow-blue-500/10">
        ...
        <div className="space-y-6 px-2">
            <div id="team-search" className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-1 flex-1 relative group">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-3 rounded-xl bg-zinc-50 dark:bg-zinc-800 border-none text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-blue-600/5 transition-all text-zinc-900 dark:text-white"
                    />
                </div>
            </div>

            {/* MOBILE-ONLY LIST VIEW */}
            <div id="team-list-mobile" className="lg:hidden grid grid-cols-1 gap-3 pb-20">
                {filteredUsers.map(u => (
                    <div key={u.id} className="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-between shadow-sm">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-[10px] font-black uppercase shrink-0 text-zinc-500">
            ...
            <div id="team-table-desktop" className="hidden lg:block bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-sm">
                    ...
                    {filteredUsers.map((u) => (
                        ...
                        <td className="px-8 py-5">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-xl bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-[10px] font-black uppercase overflow-hidden border border-zinc-200 dark:border-zinc-700 text-zinc-500">
                                    {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover" /> : u.name.substring(0,2)}
                                </div>
