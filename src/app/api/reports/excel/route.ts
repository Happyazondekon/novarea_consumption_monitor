...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Explicitly showing selected horizon in header
    mainSheet.addRow([`${resource} ANALYSIS • ${data.periodInfo.label.toUpperCase()} | Generated by: ${author}`]);
    ...
    if (includeCons) {
        ...
        consSheet.addRow([]);
        consSheet.addRow(["* Value estimated by linear interpolation (no reading captured this day)."]);
    }

    // 4. Optional Event Log (Updated with Date Column)
    if (includeEvent) {
